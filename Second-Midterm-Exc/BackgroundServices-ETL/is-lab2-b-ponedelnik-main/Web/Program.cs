...
builder.Services.AddDbContext<LegacyApplicationDbContext>(options =>
    options.UseSqlServer(
        builder.Configuration
            .GetConnectionString("LegacyRoomDb")));
...
{
    var jobKey = new JobKey("room-cleanup", "maintenance");
    options.AddJob<QuartzRoomEtlJob>(o => o.WithIdentity(jobKey));

    options.AddTrigger(o =>
    {
        o.ForJob(jobKey).WithIdentity("room-etl-job-cleanup-trigger")
            .WithCronSchedule("0 0/5 * * * ?")
            .WithDescription("Synchronizes rooms and consultations from legacy database");
    });
});
